    ...
    <div dir={isRtl ? "rtl" : "ltr"} className="min-h-screen bg-[#F8FAFC] text-[#111827]">
      ...
      <JsonLd
        data={{
          ...
          image: absoluteImageUrl(page.ogImage || page.descriptionImage),
          publisher: {
            "@type": "Organization",
            name: siteName,
          },
      ...
      <SiteHeader />
      <main>
        <CarRentalHero page={page} isRtl={isRtl} />
...
export function CarRentalHero({ page, isRtl }: { page: CarRentalPage; isRtl: boolean }) {
  return (
  ...
  return (
    <section className="mx-auto grid max-w-[88rem] gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_380px] lg:px-8">
      ...
      {page.descriptionImage ? (
        <div className="relative min-h-72 overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-sm">
          <Image
            src={resolveImagePath(page.descriptionImage)}
            alt={page.descriptionTitle || page.pageTitle}
            fill
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {cards.map((card, index) => (
          <a
            ...
            href={card.url}
            className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_24px_70px_rgb(15_23_42_/_10%)]"
          >
            {card.image ? (
